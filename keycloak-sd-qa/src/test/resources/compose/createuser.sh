...
code=$(curl -s -d "{\"username\": \"testa\", \"firstName\": \"Testa\", \"lastName\": \"Testesen\", \"email\": \"testa@organisationa.dk\", \"credentials\": [ { \"type\": \"password\", \"value\": \"Test1234\" } ], \"enabled\": true}" -H "Authorization: bearer $token" -X POST --header "Content-Type: application/json" -w "%{http_code}" http://localhost:8787/auth/admin/realms/oiosaml-organisation-a/users);
#if [ $code -ne 201 ]; then
#    echo "Could not create test user, got statuscode $code back. Failing ..."
 #   exit 1
#fi
